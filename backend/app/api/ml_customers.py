...
def customer_segments(db: Session = Depends(get_db)):
    ...
    segments = segment_customers(
        [{"customer_id": r["customer_id"], "total_spent": r["total_spent"], "total_invoices": r["total_invoices"]} for r in rows]
    )

    seg_map = {s["customer_id"]: s["segment"] for s in segments}

    for r in rows:
        r["segment"] = int(seg_map.get(r["customer_id"], 0))
