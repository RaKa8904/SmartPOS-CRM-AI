...
export default function Pricing() {
  ...
  const updatePrice = async () => {
    ...
    try {
      ...
      // refresh detected drops section for same product
            ...
            <>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // fetch drops for selected product
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4">
                  ...
                  <p className="font-semibold">{dropsData.product_name}</p>
                </div>

                <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4">
                  <p className="text-sm text-zinc-400">Current Price</p>
                  <p className="text-xl font-bold">₹ {dropsData.current_price}</p>
                </div>

                <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4">
                  <p className="text-sm text-zinc-400">Eligible Customers</p>
                  <p className="text-xl font-bold">{dropsData.count}</p>
                </div>
              </div>

              {/* Eligible customers table */}
              <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4 overflow-auto">
                <h3 className="font-semibold mb-3">Eligible Customers List</h3>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {dropsData.eligible_customers.map((c, idx) => (
                        <tr key={`${c.invoice_id}-${idx}`} className="border-b border-zinc-800">
                          <td className="py-2">
